...
def extract_png_from_csv(file_path):
    ...
    return png_entries
def inference(mode, chart_type,image_path,model,processor,sampling_params):
    if mode == 'code':
...
def main():
    ...
    parser.add_argument("--model_path", type=str, default="models/Qwen2.5-VL-32B-Instruct",
    ...
    parser.add_argument("--output_dir", type=str,
                        default="/root/paddlejob/workspace/env_run/benchmark/res-code/Qwen2.5-VL-32B_Results",
                        help="Directory to save the generation outputs.")
    parser.add_argument("--max_new_tokens", type=int, default=4096,
                        help="Maximum number of new tokens to generate.")
    parser.add_argument("--mode", type=str, default='table',
                        help="Maximum number of new tokens to generate.")
    parser.add_argument("--chart_types", type=str,  default="areachart_other,bar_other,scatter_other,line_other",  help="Comma-separated list of chart types to process. E.g. 'bar,box,violin'"
)
    ...
    # 加载模型，并指定设备为GPU
    llm = LLM(model=args.model_path,  tensor_parallel_size=4, trust_remote_code=True,gpu_memory_utilization=0.8, max_model_len=73392,limit_mm_per_prompt={"image": 1, "video": 0})
    ...
    for chart, image_path in tqdm(all_image_paths, desc="Processing all images"):
        try:
            ...
            generated_text = inference(args.mode, chart,image_path,llm,processor,sampling_params)
