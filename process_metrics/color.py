...
def to_hex(c: Any) -> str:
    """
    支持 '#RRGGBB', 'rgba(...)', 'rgb(...)', 英文名, RGB元组，或 colormap 名
    返回大写 HEX；无法解析的保持原样
    """
    if isinstance(c, str):
        if c.startswith('#'):
...
def get_nested_value(d, key_path):
    """
    给定dict和嵌套key路径，逐层取值（带空值检验）
    
    参数:
        d: 输入的字典
        key_path: 用点号分隔的嵌套键路径（如 "a.b.c"）
        
    返回:
        (key_path, 处理后的值) 或 None（如果路径不存在或遇到None值）
    """
    if not d or not key_path:  # 空字典或空路径检查
        return None
    ...
    try:
        ...
        if isinstance(current, list):
            if current:  # 非空列表检查
                ...
                return (key_path, [])  # 空列表返回空列表
        
        return (key_path, to_hex(current)) if current is not None else None
    
    except (IndexError, AttributeError, TypeError):  # 捕获可能的异常
        return None
...
def get_color_from_layout_or_template(layout, key, rename=None):
    """优先从layout取key，否则从template取；返回(改名后的key, 色值)。"""
...
def get_all_color(fig,  all_colors = [],fallback_keys: Optional[List[str]] = None) -> List[Tuple[str, Any]]:
    """
    返回 [(color_key, color_value)] 列表。
    mode='gold' 时不做 fallback；mode='gen' 时仅对在 fallback_keys 中的 key 做模板 fallback。
    """
        ...
        for entry in fig.data:
            ...
            for color_name in v:
                ...
                if fallback_keys:
                    if color_name not in fallback_keys:
                        continue
                    trace_type = entry_dict.get('type')  # e.g. "scatter", "bar", ...
                    # 从 template.data 里拿到该类型的默认 trace 列表
...
def is_color_or_colorscale_valid(value: Any) -> bool:
    """
    判断一个值是单色 or 色带（colorscale），并检查是否合法。
    """
...
def hex_color_similarity(c1: str, c2: str) -> float:
    """
    计算两个 HEX 颜色的感知相似度，返回 ΔE₀₀ 值，越小越相似。
    """
    rgb1 = to_rgb(c1)  # [0,1]
    rgb2 = to_rgb(c2)
...
def compute_precision_hungarian(gold_colors: List[str], gen_colors: List[str], threshold: float = 0.0) -> float:
    """
    使用匈牙利算法计算颜色匹配的 precision。
    - 会自动过滤无效颜色。
    - 使用 calculate_similarity_single 进行相似度计算。
    """
    # 提取实际颜色部分（处理 "--#xxxxxx" 格式）并过滤无效颜色   
    ...
    for i, g1 in enumerate(gen_colors):
        for j, g2 in enumerate(gold_colors):
            sim_matrix[i, j] = calculate_similarity_single(g1, g2)  # 你已有此函数
